...
output_dir = 'analyses/plots/flex-lines-parts'
...
# Global variables for row range (excluding the header)
start_row = 100
end_row = 200
num_rows = end_row - start_row

# Number of neurons per plot
neurons_per_plot = 100
...
for layer_file in layers:
    ...
    typo_activations = pd.read_csv(typo_path, header=0).iloc[start_row:end_row]
    clean_activations = pd.read_csv(clean_path, header=0).iloc[start_row:end_row]
    ...
    for start in range(0, total_neurons, neurons_per_plot):
        ...
        plt.title(f'Neuron Activations {start}-{end} ({layer_file.split('.')[0]})')
        plt.xlabel('Neuron Index')
        plt.ylabel('Activation Value')
        plt.legend()

        # Save plot
        output_path = os.path.join(output_dir, f'activations_{start}-{end}_{layer_file.split('.')[0]}.png')
